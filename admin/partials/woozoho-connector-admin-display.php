    ...
    <form method="post" name="woozoho-connector_options" action="options.php">
        ...
        <table class="form-table">
            ...
            <tr>
                ...
                <td><input type="checkbox" id="<?php echo $this->plugin_name; ?>-enable_logging" name="<?php echo $this->plugin_name; ?>[enable_logging]" value="1" <?php checked($options["enable_logging"], 1); ?>/></td>
            </tr>
            <tr>
                <th scope="row"><label for="zoho_key">Zoho API Key</label></th>
                <td><input type="text" class="regular-text" id="<?php echo $this->plugin_name; ?>-zoho_key" name="<?php echo $this->plugin_name; ?>[zoho_key]" value="<?php echo $options["zoho_key"] ?>"/>
                </td>
            </tr>
            <tr>
                <th scope="row"><label for="zoho_organisation_id">Zoho Organisation ID</label></th>
                <td><input type="text" class="regular-text" id="<?php echo $this->plugin_name; ?>-zoho_organisation_id" name="<?php echo $this->plugin_name; ?>[zoho_organisation_id]" value="<?php echo $options["zoho_organisation_id"] ?>" /></td>
            </tr>
            <tr>
                <th scope="row"><label for="zoho_organisation_id">Sync Orders Every:</label></th>
                <td><input type="text" class="regular-text" id="<?php echo $this->plugin_name; ?>-sync_orders_interval" name="<?php echo $this->plugin_name; ?>[sync_orders_interval]" value="<?php echo $options["sync_orders_interval"] ?>" /></td>
            </tr>
            </tbody>
        </table>
...
if($_GET["find"] == "dup")
{
	...
	while ( $loop->have_posts() ) : $loop->the_post();
		...
		if($query->post_count > 1 && $sku != NULL) {
		    echo 'happen.';
		    $duplicates[$sku][] = array('ID' => $theid, 'PRICE' => $product->get_price(), 'TITEL' => $thetitle, 'PARENT:' => $parent, 'PARENT_STATUS' => $parent_post_status );
		}
